...
void MPCCartOptClass::optThreadFunc()
{
    ...
    */
    ...
    // blocking
    Map<MatrixXd,0,Stride<FEAT_BLOCK_SZ,MPC_N_FEAT> > optXDiff(optDiff.data(),MPC_BLOCK_SZ,MPC_HORIZON_BLOCKS);
    Map<MatrixXd,0,Stride<FEAT_BLOCK_SZ,MPC_N_FEAT> > optZDiff(&optDiff.data()[1],MPC_BLOCK_SZ,MPC_HORIZON_BLOCKS);

    Map<MatrixXd,0,Stride<FEAT_BLOCK_SZ,MPC_N_FEAT> > optXPos(optFutPos.data(),MPC_BLOCK_SZ,MPC_HORIZON_BLOCKS);
    Map<MatrixXd,0,Stride<FEAT_BLOCK_SZ,MPC_N_FEAT> > optZPos(&optFutPos.data()[1],MPC_BLOCK_SZ,MPC_HORIZON_BLOCKS);

    Map<VectorXd,0,InnerStride<MPC_N_FEAT> > optXF(optForces.data(),MPC_BLOCK_SZ*MPC_HORIZON_BLOCKS);
    Map<VectorXd,0,InnerStride<MPC_N_FEAT> > optZF(&optForces.data()[1],MPC_BLOCK_SZ*MPC_HORIZON_BLOCKS);

    Map<VectorXd,0,InnerStride<MPC_N_FEAT> > aFGradX(aFGrad.data(),MPC_BLOCK_SZ*MPC_HORIZON_BLOCKS);
    Map<VectorXd,0,InnerStride<MPC_N_FEAT> > aFGradZ(&aFGrad.data()[1],MPC_BLOCK_SZ*MPC_HORIZON_BLOCKS);

    Map<MatrixXd,0,Stride<1,MPC_N_FEAT> > aFMat(optForces.data(),MPC_BLOCK_SZ*MPC_HORIZON_BLOCKS,MPC_N_FEAT);
    Map<MatrixXd,0,Stride<1,MPC_N_FEAT> > aFGMat(aFGrad.data(),MPC_BLOCK_SZ*MPC_HORIZON_BLOCKS,MPC_N_FEAT);
    Map<MatrixXd,0,Stride<1,MPC_N_FEAT> > gsMat(gradScale.data(),MPC_BLOCK_SZ*MPC_HORIZON_BLOCKS,MPC_N_FEAT);
    Map<MatrixXd,0,Stride<1,MPC_N_FEAT> > optFutPosMat(optFutPos.data(),MPC_BLOCK_SZ*MPC_HORIZON_BLOCKS,MPC_N_FEAT);
    ...
    optFutFFeat.head(optStepInBlock*MPC_N_FEAT) = futFFeat.head(optStepInBlock*MPC_N_FEAT);
    optFutFFeat.tail((MPC_BLOCK_SZ-optStepInBlock)*MPC_N_FEAT) = optForces.segment(optStepInBlock*MPC_N_FEAT,(MPC_BLOCK_SZ-optStepInBlock)*MPC_N_FEAT);
    ...
    */
    ...
    mpcShMem->cutDist =  optPrevPos(1) - optCutDist;
    ...
    while(true)
    {
        ...
        for(int i = 0; i < MPC_HORIZON_BLOCKS; i++)
        {
            ...
            if(i == 0)
            {
                aCurR.head(optStepInBlock*MPC_N_FEAT) = optFutPFeat.head(optStepInBlock*MPC_N_FEAT);
            }
            ...
            aPrevPos = aCurR.tail(MPC_N_FEAT);
        ...
        {
            optDiff.block<MPC_N_FEAT,1>(FEAT_BLOCK_SZ-MPC_N_FEAT,i) += aAccumDiff;
            // NEEDS TO CHANGE IF FEATS DO
            if(i > 0)
            {
                aAccumDiff(0) = optXDiff.block<MPC_BLOCK_SZ,1>(0,i).sum();
                aAccumDiff(1) = optZDiff.block<MPC_BLOCK_SZ,1>(0,i).sum();
            }
        ...
        aFGradX(0) += smCost*(optXF(0) - optCurFeat(optCurFeat.size()-2));
        ...
        if(!(mpcShMem->curStepInBlock == optStepInBlock) || mpcShMem->blockOffset > 0 || mpcShMem->curMode == MODE_NORMAL)
        {
            ...
            if(mpcShMem->blockOffset > 0)
            {
                ...
                // better?
                optForces.tail(FEAT_BLOCK_SZ) = optForces.tail(MPC_N_FEAT).replicate(MPC_BLOCK_SZ,1);
