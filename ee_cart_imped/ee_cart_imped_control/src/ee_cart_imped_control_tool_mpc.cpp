...
bool EECartImpedControlClassToolMPC::init(pr2_mechanism_model::RobotState *robot,
  ...
  bumpMPC = 0;
    ...
    node_.param("xBarrScale",mpcShMem->xBarrScale,X_BARR_SCALE);
      mpcShMem->curMode = MODE_NORMAL;
      curMPCF.fill(0.0);
      ROS_INFO("EE: SETTING 1");
      mpcShMem->ctrlReady = 1;
      ROS_INFO("EE: READY: %d",mpcShMem->ctrlReady);
      sLock.unlock();
      }

      ros::Rate optPollRate(10);

      while(mpcShMem->optReady != 1)
      {
        optPollRate.sleep();
      }
...
void EECartImpedControlClassToolMPC::update()
{
    ...
    { // TRY TO TAKE LOCK
        ...
        if(sLock)
        {
            curMPCF = -mpcForces.segment(mpcShMem->blockOffset*MPC_BLOCK_SZ+mpcShMem->curStepInBlock*3,3);
            sLock.unlock();
        }
    } // RELEASE LOCK
